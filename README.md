...
2) Add a new `step` on your workflow code below `actions/checkout`:
...
jobs:
  build:
    ...
    steps:
      ...
      - uses: freenet-actions/ms-teams-deploy-card@v4.0.0
        if: always()
        with:
...
- As much as possible, always set this Github action right after `actions/checkout` and before any job steps. The following diagram shows when this job if going to trigger if done the right way.
